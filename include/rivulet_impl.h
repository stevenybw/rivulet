struct Serdes
{
  ...
  template <>
  static string stream_deserialize<string>(InputChannel* in) {
  ...
  template <>
  static double stream_deserialize<double>(InputChannel* in) {
    double* ptr = in->pull(sizeof(double));
    return *ptr;
  }
  ...
  template <typename T>
  static TS<T> stream_deserialize<TS<T>>(InputChannel* in) {
    Header* buf = (Header*) in->pull(sizeof(Header));
    assert(buf->type == TYPE_TS);
    size_t ts = buf->bytes;
    T elem = stream_deserialize<T>(in);
  ...
  static void stream_serialize(OutputChannel* out, TS<T>& elem) {
    ...
    out->push(&header, sizeof(header));
    stream_serialize<T>(out, elem.e);
  }

  template <typename T>
  static WN<T> stream_deserialize<WN<T>>(InputChannel* in) {
    Header* buf = (Header*) in->pull(sizeof(Header));
